...
from astar_quadtree import astar_quadtree
...
filename = 'example_map_occupancy.png'
...
def plan(event):
    ...
    if click == 0:
        ...
        path, path_px = astar_quadtree(start_node, goal_node, qtmap, movement='8N')
        if path_px:
        ...
        if path_px:
            ...
            draw_listOfTiles_on_canvas(path_px, cv, color="purple", width=1)
        else:
            print('Goal is not reachable')


def plot_on_canvas(path, cv, color="purple", width=3):
    # plot path
...
if __name__ == '__main__':
    ...
    img = PhotoImage(file= 'C:/CUICAR/AuE8930/HW/HW_3/PartB/example_map_occupancy.png')
