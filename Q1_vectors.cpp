...
void printStudents(vector<Student> students) {
    ...
    std::vector<Student>::iterator itrStruct;

    for (itrStruct = students.begin(); itrStruct != students.end(); itrStruct++)
    {
        cout << itrStruct->firstName << " " << itrStruct->lastName << ", " << itrStruct->studentID << ", " << floor(itrStruct->GPA * 100 + 0.5) / 100 << endl;
    }
...
vector<Student> delStudent(vector<Student> students) {
    ...
    for (itrStruct = students.begin(); itrStruct != students.end(); itrStruct++)
    {
        ...
        if (itrStruct->studentID != studentIDtoDel)
        {
            
            newStudents.push_back(*itrStruct);
        }
    }
    students.clear();
    return newStudents;
